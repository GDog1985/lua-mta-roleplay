...
addEventHandler( getResourceName( resource ) .. ":cegui:characters", root,
	function( )
		...
		local result, num_rows = exports.database:query( "SELECT `name`, DATEDIFF(`last_login`, NOW()) AS `last_login` FROM `characters` WHERE `account_id` = ?", getElementData( client, "client:id" ) )
